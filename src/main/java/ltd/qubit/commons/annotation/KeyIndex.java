 ...
 * 用到的列分别为{@code owner_type}, {@code owner_id}。具体SQL语句大致如下：</p>
 *
 * <pre><code>
 *   INDEX `owner` (`owner_type`, `owner_id`)
 * </code></pre>
 *
 * <p>为了能正确对该索引的值表示为字符串，我们需要在{@code owner.type}和{@code owner.id}
 * 字段定义上分别标注{@code @KeyIndex(0)}和{@code @KeyIndex(1)}。Java代码大致如下：</p>
 *
 * <pre><code>
 * class Owner {
 *    @KeyIndex(0)
